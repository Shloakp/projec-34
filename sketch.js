...
ball = Matter.Bodies.circle(625,100,20,ball_options);
...
function draw() {
  ...
  ellipse(ball.position.x,ball.position.y,radius,radius);
   ...
   text("Player 2 Score: "+p2Score,900,100)
grnd1.show();
grnd2.show();
grnd3.show();
grnd4.show();
...
if(keyDown(RIGHT_ARROW)){
  ...
  if(keyIsDown(65)){
   ...
   if(ball.position.x <= 210 && ball.position.y>=600){
     p2Score +=1
     reset();
    } 

   if(ball.position.x >=990 && ball.position.y>=600){
    p1Score +=1
    reset(); 
  ...
  if(keyIsDown(UP_ARROW)&&player1.position.x == ball.position.x){
    ...
    if(keyIsDown(32)&&player2.position.x == ball.position.x){
      Matter.Body.applyForce(ball,{x:0,y:0},{x:-40,y:0});
      Matter.Body.applyForce(ball,{x:0,y:0},{x:0, y:-100});
    }
